...
class BasicScanForm extends Component {
  _isMounted = false
  constructor(props) {
    super(props);
    this.state = {
        ...
        <Form className="scan-form" onSubmit={this.handleSubmit}>
            ...
            <FormItem label="Box Name">
              {getFieldDecorator("name", {
                ...
                <Input
                  addonBefore={this.props.usePrefix ? 
                  this.state.currentPrefix ? boxSelect :
            ...
            <div style={{margin: '1em 0px'}}>
              <Button onClick={this.handleSubmit} type="primary" block>Scan</Button>
            </div>
          </Form>
          ...
          <div>
            {!this.props.hideScanLog && (
              <div id="scan-log">
                <Button
                  className="no-bg no-border no-margin"
                  type="link"
                  size="small"
                  onClick={() => this.setState({ logOpen: !this.state.logOpen })}
                >
                  Recent Scans {this.state.logOpen ? <Icon type="up" /> : <Icon type="down" />}
                </Button>
              </div>
            )}
          </div>
...
const ScanForm = Form.create()(BasicScanForm);
